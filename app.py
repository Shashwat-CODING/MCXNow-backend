    ...
    for stock in array2:
        if stock.get('Symbol'):
            ...
            if symbol in stocks_dict:
                # Stock exists in both APIs - mark as dual source
                stocks_dict[symbol]['sources'].append('api2')
                stocks_dict[symbol]['source'] = 'both'
                # You can choose to keep API1 data, API2 data, or merge fields
                # Here we'll add API2 specific fields with prefix
...
def get_rate_changed_stocks(current_data, previous_data):
    ...
    # Look for rate changes in existing stocks
    for symbol, current_stock in current_stocks.items():
        ...
        # Only include if rate has changed
    ...
    while True:
        try:
            ...
            if current_data_api1 or current_data_api2:
                ...
                # Compare with previous combined data
